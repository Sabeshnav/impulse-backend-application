...
class MyTests(Baseview, View):
    ...
    def get(self, request):
        ...
        if self.retrieve("*", "teacher", "(token = \"" + token + "\")"):
            ...
            test_details = self.retrieve("*", "test", "(teacher_id = \"" + str(teacher_id) + "\" AND class_id = \""+ class_id + "\" AND subject_id = \""+ subject_id + "\")")
    ...
    def get(self, request):
        ...
        if self.retrieve("*", "teacher", "(token = \"" + token + "\")"):
            ...
            contents = self.retrieve("*", "class_content", "(teacher_id = \"" + str(
                teacher_id) + "\" AND class_id = \"" + class_id + "\" AND subject_id = \"" + subject_id + "\") ORDER BY id DESC")
            for i in contents:
                ids.append(str(i[0]))
                o = i[4]
                o = json.loads(o)
