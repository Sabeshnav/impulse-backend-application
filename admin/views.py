...
class Mentor(View, Baseview, Tables):
    ...
    def post(self, request):
        ...
        if temp[0][0]:
            self.update("mentor = null", "teacher", "(id = " + str(temp[0][0]) + ")")
            self.update("mentor = " + teacher_id, "class", "(id = " + class_id + ")")
            self.update("mentor = " + class_id, "teacher", "(id = " + teacher_id + ")")
        else:
            self.update("mentor = " + teacher_id, "class", "(id = " + class_id + ")")
            self.update("mentor = " + class_id, "teacher", "(id = " + teacher_id + ")")
        ...
        if self.retrieve("*", "teacher", "(name = \"" + username + "\")"):
            ...
            while flag:
                ...
                if self.retrieve("*", "teacher", "(token=\"" + token + "\")"):
                    ...
                    flag = False
            self.insert("teacher", "(name, phone, email, subjects_ids, class_ids, password, token)",
                        "(\"" + username + "\", \"" + phone + "\", \"" + email + "\", '{\"subject_ids\": \"" + subject_ids + "\"}', '{\"class_ids\": \"\"}', \"" + password + "\", \"" + token + "\")")
...
class ClassRoom(View, Baseview, Tables):
    ...
    def post(self, request, class_id):
        ...
        if self.is_exists("class", "*", "(name = \""+class_name+"\")"):
            ...
            return HttpResponse(self.insert("class", "(name, mentor, teachers, students)", "(\"" + class_name + "\", " + str(mentor) + ", '{\"teachers\": \"" + str(mentor) + "\"}', '{\"students\": \"\"}')"), status=200)
    ...
    def get(self, request, class_id):
        ...
        for row in res:
            ...
            teachers.append(row[3])
            students.append(row[4])
